...
for i in range(filesNum):
  ...
  for j in range(filesNum):
    ...
    if data['rawData']['clinician'] == dataCompare['rawData']['clinician']:
      totalAnnotation = totalAnnotation + dataCompare["rawData"]["stacks"][0]["seriesDescription"];
    j = j + 1;
  print("The Total Annotation of " + data["rawData"]["stacks"][0]["seriesDescription"] + " is ", totalAnnotation);
  totalAnnotation = 0;
  i = i + 1;
