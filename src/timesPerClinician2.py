...
for fileName in json_files:
	...
	nameAndTimePairs.append([key,val])

sumsPerDoc = []
for i,j in itertools.groupby(sorted(nameAndTimePairs,key=lambda x:x[0]) , lambda x:x[0]):
  l = list(j)
  sumsPerDoc.append([i,sum([int(x[1]) for x in l])])
